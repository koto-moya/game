...
def stable(steps=10, prompt="a car sized frog", out="image/image.png", 
          noshow=False, fp16=False, timing=False, seed=None, guidance=7.5):
    """Run stable diffusion with given parameters and return the generated image."""
    Tensor.no_grad = True
    ...
    prompt_tensor = Tensor([tokenizer.encode(prompt)])
    context = model.cond_stage_model.transformer.text_model(prompt_tensor).realize()
    print("got CLIP context", context.shape)

    prompt_tensor = Tensor([tokenizer.encode("")])
    unconditional_context = model.cond_stage_model.transformer.text_model(prompt_tensor).realize()
    ...
    with Context(BEAM=getenv("LATEBEAM")):
        for index, timestep in (t:=tqdm(list(enumerate(timesteps))[::-1])):
            ...
            with Timing("step in ", enabled=timing, on_exit=lambda _: f", using {GlobalCounters.mem_used/1e9:.2f} GB"):
                tid = Tensor([index])
                latent = run(model, unconditional_context, context, latent, Tensor([timestep]), alphas[tid], alphas_prev[tid], Tensor([guidance]))
    ...
    im.save(out)
    # Open image.
    if not noshow: im.show()

    # validation!
    default_prompt = "a horse sized cat eating a bagel"
    if prompt == default_prompt and steps == 6 and seed == 0 and guidance == 7.5:
